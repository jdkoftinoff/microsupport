...
int us_webapp_json_reply(us_http_response_header_t *header, us_buffer_t *buf, const us_json_t *m) {
    ...
    r &= us_http_response_header_init_ok(header, 200, "application/json", (int)us_buffer_readable_count(buf), true);
...
us_webapp_t *us_webapp_json_create(us_allocator_t *allocator,
                                   const char *path,
...
void us_webapp_json_destroy(us_webapp_t *self_) {
...
int us_webapp_json_dispatch(us_webapp_t *self_,
