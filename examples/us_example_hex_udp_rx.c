...
int main(int argc, char **argv) {
    ...
    if (argc == 2 && strcmp("--help", argv[1]) == 0) {
        fprintf(stdout, "usage:\n\t%s {local ip} {local port}\ndefaults to: %s %s\n", argv[0], listen_host, listen_port);
        exit(1);
    }
    if (argc > 1) {
        listen_host = argv[1];
    ...
    if (listen_ai) {
        ...
        while (true) {
            ...
            do {
                ...
                pkt_len = recvfrom(
                    incoming_sock_fd, &pkt_buf, sizeof(pkt_buf), 0, (struct sockaddr *)&remote_addr, &remote_addr_len);
            } while (pkt_len < 0 && errno == EINTR);
            if (pkt_len < 0) {
                perror("recvfrom:");
                break;
            }
            e = getnameinfo((struct sockaddr *)&remote_addr,
                            remote_addr_len,
                            hostname_buf,
